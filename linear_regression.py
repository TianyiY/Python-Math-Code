...
if __name__ == "__main__":
    ...
    for t in range(4):
        ...
        for i, d in enumerate(d_pool):
            ...
            output = u'%s：%dorder，Parameters：' % (titles[t], d)
            if hasattr(lin, 'alpha_'):
                idx = output.find(u'Parameter')
                output = output[:idx] + (u'alpha=%.6f，' % lin.alpha_) + output[idx:]
            if hasattr(lin, 'l1_ratio_'):   # According to cross validation，from l1_ratio(list)select optimal l1_ratio_(float)
                idx = output.find(u'Parameter')
            ...
            # print 'R2 and corr_coef：', r2, corr_coef
            # print 'R2：', s, '\n'
            z = N - 1 if (d == 2) else 0
            label = u'%dorder，$R^2$=%.3f' % (d, s)
    ...
    plt.suptitle(u'poly fit', fontsize=22)
    plt.show()
    ...
    plt.xlabel(u'test：LinearRegression/Ridge/LASSO/Elastic Net', fontsize=15)
    plt.ylabel(u'XSS value', fontsize=15)
    plt.title(u'total sum of square TSS=？', fontsize=18)
    plt.grid(True)
    plt.show()